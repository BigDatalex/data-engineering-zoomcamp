...
- `select * from myproject.raw_nyc_tripdata.ext_green_taxi` x
...
- Update the WHERE clause to `pickup_datetime >= CURRENT_DATE - INTERVAL '{{ var("days_back", env_var("DAYS_BACK", "30")) }}' DAY` x
...
- `dbt run --select models/staging/+` x
...
WITH revenue_by_quarter AS (
    -- Step 1: Compute total revenue per service type and quarter
    SELECT 
        SUM(total_amount) AS total_revenue, 
        pickup_quarter, 
        service_type,
        pickup_year
    FROM `splendid-parsec-449218-j6.dbt_aflick.fact_trips`
    WHERE pickup_year BETWEEN 2019 AND 2020
    GROUP BY pickup_quarter, pickup_year, service_type
),
revenue_with_lag AS (
    -- Step 2: Join with itself to get the previous year's revenue
    SELECT 
        curr.service_type,
        curr.pickup_quarter,
        curr.pickup_year,
    ...
    FROM revenue_by_quarter curr
    LEFT JOIN revenue_by_quarter prev
...
SELECT 
    ...
    current_revenue,
    previous_revenue,
    ROUND(((current_revenue - previous_revenue) / previous_revenue) * 100, 2) AS yoy_growth_percentage
FROM revenue_with_lag
WHERE previous_revenue IS NOT NULL
ORDER BY service_type, pickup_quarter;
```
...
WITH trip_data AS (
    ...
    SELECT 
        ...
        pickup_month,
        fare_amount
    FROM `dbt_aflick.fact_trips`
    WHERE fare_amount > 0 
      AND trip_distance > 0 
      AND payment_type_description IN ('Cash', 'Credit Card')
),
percentile_calc AS (
    -- Step 2: Compute percentiles for each partition
    SELECT 
        service_type,
        pickup_year,
        pickup_month,
...
--SELECT DISTINCT service_type, pickup_year, pickup_month, p97, p95, p90 from percentile_calc
-- Step 3: Filter only for April 2020 (Year = 2020, Month = 4)
...
- green: {p97: 55.0, p95: 45.0, p90: 26.5}, yellow: {p97: 31.5, p95: 25.5, p90: 19.0} x
- green: {p97: 40.0, p95: 33.0, p90: 24.5}, yellow: {p97: 52.0, p95: 37.0, p90: 25.5}
- green: {p97: 40.0, p95: 33.0, p90: 24.5}, yellow: {p97: 31.5, p95: 25.5, p90: 19.0} x
...
WITH tripdata AS (
    ...
    FROM `splendid-parsec-449218-j6.dbt_aflick.fact_trips_fhv`
),
percentile_calc AS (
    -- Step 2: Calculate 90th percentile of trip duration
    SELECT 
        *, 
...
filtered AS (
    -- Step 3: Select distinct results for specific pickup zones and year/month
    SELECT DISTINCT 
        pickup_year, 
        pickup_month,
        pickup_zone, 
...
ranked AS (
    -- Step 4: Rank the results by p90 within each pickup zone, year, and month
    SELECT 
        *, 
        ROW_NUMBER() OVER (PARTITION BY pickup_zone, pickup_year, pickup_month ORDER BY p90 DESC) AS rank 
    FROM filtered
)
-- Step 5: Select the 2nd longest p90
SELECT * 
FROM ranked 
WHERE rank = 2;
```
...
- LaGuardia Airport, Chinatown, Garment District x
